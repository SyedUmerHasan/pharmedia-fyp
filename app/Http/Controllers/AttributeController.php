...
class AttributeController extends Controller
{
    ...
    public function create()
    {
        ...
        return view('admin.admin_pages.Attribute.create_attribute')->with(compact('GroupAttributes'));
    }
    public function save(Request $request)
    {
        if ($request->isMethod('post')) {
            ...
            $ProductAttribute->product_id = (isset($request->ProductID) ? $request->ProductID: '10' );
            // Change product ID
            $ProductAttribute->user_id = $user_id;
            $ProductAttribute->attribute_group_id = $request->AttributeGroup;
            $ProductAttribute->save();

            session()->flash('success', 'Group Attributes has been successfully added');
    ...
    public function edit($attribute_id)
    {
        ...
        return view('admin.admin_pages.Attribute.edit_attribute')->with(compact('attributes'))->with(compact('GroupAttributes'));
    }
    ...
    public function update(Request $request, $attribute_id)
    {
        if ($request->isMethod('post')) {
            ...
            $ProductAttribute->attributes_name = $request->AttributeName;
            $ProductAttribute->product_id = 10;
            // Change product ID
            $ProductAttribute->user_id = $user_id;
            $ProductAttribute->attribute_group_id = $request->AttributeGroup;
            $ProductAttribute->save();
